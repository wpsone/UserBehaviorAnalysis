...
object UvWithBloom {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val uvStream: DataStream[UvCount] = env.readTextFile(resource.getPath)
      .map(data => {
        val dataArray: Array[String] = data.split(",")
        UserBehavior(dataArray(0).trim.toLong, dataArray(1).trim.toLong, dataArray(2).toInt, dataArray(3).trim, dataArray(4).trim.toLong)
      })
...
class Bloom(size:Long) extends Serializable{
  //位图的总大小 16MB=16*2^10KB=16*2^10*2*10B=16*2^10*2^10*1000byte
  private val cap = if (size > 0) size else 1 << 27

  //定义hash函数 seed一般为质数，可以保证较为分散，允许的范围内大一些好
